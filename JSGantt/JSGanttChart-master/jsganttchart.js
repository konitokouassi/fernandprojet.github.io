  ...
  var root = window,
      ...
      normalize: function () {
        ...
        if (!this.has("endDate")) {
          ...
          if (this.has("duration")) {
            this.get("endDate").setDate(
              this.get("startDate").getDate() + this.get("duration")
            );
            this.unset("duration");
          } else {
        ...
        if (this.has("elements")) {
          this.collection.add(
            _(this.get("elements")).map(function (el) {
    ...
    GanttElementCollection = Backbone.Collection.extend({
      ...
      sort: function (model) {
        var this_ = this;
        this.reset(
          collection.sortBy(function (model) {
            var order;
            if (
              model.has("parentElement") &&
              this_.get(model.get("parentElement"))
            ) {
              /// HACK
              order =
                this_.get(model.get("parentElement")).get("order") +
                (0.00001 * model.get("order") + 0.00001);
            } else {
                ...
                _(this_.options.fields).map(function (field) {
                  ...
                  if (
                    ...
                    str = _(model.get(field)).reduce(function (memo, resource) {
                      return (
                        (memo ? memo + ", " : "") +
                        this_.options.resources[resource]
                      );
                    }, "");
    ...
    GanttElementView = Backbone.View.extend({
      ...
      render: function () {
        var this_ = this,
          ...
          noOfDays = Math.round(
            (model.get("endDate").getTime() -
              model.get("startDate").getTime()) /
              (24 * 60 * 60 * 1000)
          ),
          dayFromStart = Math.round(
            (model.get("startDate").getTime() -
              this.options.firstDate.getTime()) /
              (24 * 60 * 60 * 1000)
          ),
          el;
        ...
        if (
          model.has("type") &&
          this.options.types.hasOwnProperty(model.get("type"))
        ) {
          this.$el.css({
            borderBottomColor: this.options.types[model.get("type")].color,
          });
        }
        ...
        if (model.has("percentageDone") && model.get("percentageDone") > 0) {
          ...
          this.$el.append(
            ...
            $(
              '<div class="donetext">' +
                (model.get("percentageDone") < 100
                  ? model.get("percentageDone") + "% done"
                  : "Done") +
                "</div>"
            )
          );
        ...
        if (model.has("slackEndDate")) {
          ...
          noOfDays = Math.round(
            (model.get("slackEndDate").getTime() -
              model.get("endDate").getTime()) /
              (24 * 60 * 60 * 1000)
          );
        ...
        if (model.has("predecessors")) {
          $.fn.append.apply(
            this.$el,
            _(model.get("predecessors")).map(function (predecessor) {
              var predecessorModel = this_.options.collection.get(predecessor);
              if (predecessorModel) {
                el = $(
                  '<div class="arrowline"><div class="arrowhead"></div></div>'
                );
                noOfDays = Math.round(
                  (model.get("startDate").getTime() -
                    predecessorModel.get("endDate").getTime()) /
                    (24 * 60 * 60 * 1000)
                );
                var noOfRows =
                  collection.indexOf(model) -
                  collection.indexOf(predecessorModel);
                el.css({
                  right: "100%",
                  bottom: "100%",
                  width: noOfDays * 23,
                  height: noOfRows * 17 - 5,
                });
                return el;
              }
            })
      ...
      render: function () {
        ...
        while (dateIterator <= lastDate) {
          if (dateIterator.getMonth() !== currMonth) {
            ...
            currMonthEl = $(
              "<th>" +
                monthNames[dateIterator.getMonth()] +
                " " +
        ...
        $.fn.append.apply(
          ...
          this.options.collection.map(function (model) {
            var row = $('<tr class="jsgt_' + model.get("id") + '"></tr>'),
              ...
              dateIterator = new Date(firstDate.getTime()),
              elementHolder = $(
                '<div class="gantt-element-holder">&nbsp;</div>'
              ),
    ...
    KeyView = Backbone.View.extend({
      ...
      render: function () {
        ...
        $.fn.append.apply(
          this.$el,
          _(this.options.types).map(function (type) {
            return (
    ...
    GanttContainerView = Backbone.View.extend({
      ...
      render: function () {
        ...
        setTimeout(function () {
          ...
          this_.ganttView.$el.css({
            marginLeft: this_.dataView.$el.outerWidth(),
          });
        });
        return this;
      },
